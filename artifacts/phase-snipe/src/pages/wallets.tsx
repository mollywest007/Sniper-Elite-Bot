...
function CopyButton({ text, label = "Copied!" }: { text: string; label?: string }) {
  ...
  return (
    <button
      ...
      className="flex items-center gap-2 px-3 py-2 rounded-md text-[10px] font-bold tracking-widest uppercase bg-accent/50 border border-border text-muted-foreground hover:text-foreground hover:bg-accent transition-all"
      title="Copy"
    >
  ...
  return (
    ...
    >
      <CardContent className="p-0">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between p-6 border-b border-border/50 bg-card/40 relative overflow-hidden gap-4">
          ...
          <div className="flex items-center gap-4 z-10">
            ...
            <div>
              <div className="flex items-center gap-3">
                <h3 className="font-bold text-xl tracking-tight text-foreground">{wallet.name}</h3>
                {wallet.isActive && (
              ...
              <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mt-1 flex items-center gap-1.5">
                <Terminal className="h-3 w-3" /> Solana Node
              </div>
            </div>
          </div>
          <div className="sm:text-right z-10 border-t sm:border-none border-border/50 pt-4 sm:pt-0">
            <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Available Funds</div>
            <div className=" font-bold text-2xl text-foreground">
              {formatSol(wallet.balanceSol)} <span className="text-sm font-normal text-primary font-bold">SOL</span>
            </div>
            <div className="text-[10px] font-bold  text-muted-foreground tracking-widest">{formatUsd(wallet.balanceUsdc)}</div>
          </div>
        </div>
        ...
        <div className="p-6 border-b border-border/50">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <div className="flex-1 bg-background/50 rounded-lg border border-border px-4 py-3 min-w-0 w-full">
              <p className=" text-sm text-foreground break-all select-all leading-relaxed">
                {wallet.address}
              </p>
            </div>
        ...
        {wallet.privateKey && (
          <div className="p-6 border-b border-border/50 bg-destructive/5 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="text-[10px] font-bold text-destructive uppercase tracking-widest flex items-center gap-2">
                <Key className="h-3.5 w-3.5" /> Private Key Extract
              </div>
              <span className="text-[9px] font-bold tracking-widest uppercase text-destructive/80 bg-destructive/10 border border-destructive/20 px-2 py-1 rounded">
                ⚠️ Secure Material
              </span>
            </div>
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
              <div className="flex-1 bg-background/80 rounded-lg border border-destructive/20 px-4 py-3 min-w-0 w-full">
                {showKey ? (
                  <p className=" text-sm text-destructive break-all select-all leading-relaxed">
                    {wallet.privateKey}
                  </p>
                ) : (
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                <Button
                  ...
                  className="text-[10px] font-bold tracking-widest uppercase border-border hover:bg-accent flex-1 sm:flex-none h-11"
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            {!wallet.isActive && (
              <Button
                variant="outline"
                size="sm"
                className="text-[10px] font-bold tracking-widest uppercase border-primary/30 text-primary hover:bg-primary/10 flex-1 sm:flex-none h-10"
                onClick={onActivate}
              >
                Set Active
              </Button>
            ...
            <Button
              variant="ghost"
              size="sm"
              className="text-[10px] font-bold tracking-widest uppercase text-muted-foreground hover:text-destructive hover:bg-destructive/10 flex-1 sm:flex-none h-10"
              onClick={onDelete}
            >
    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col items-center justify-center py-16">
          <Card className="w-full max-w-2xl border-none hero-gradient corner-brackets relative overflow-hidden">
            ...
            <CardContent className="p-12 flex flex-col items-center text-center gap-8 relative z-10">
              ...
              <div>
                <h2 className="text-3xl font-black uppercase tracking-widest mb-3 text-foreground">
              ...
              {hasWallets && (
                <div className="w-full bg-background/80 border border-border rounded-xl px-6 py-5 text-left flex flex-col gap-2">
                  ...
                  <div className=" text-primary font-bold text-sm tracking-widest break-all">
                    {wallets[0].address.slice(0, 16)}••••{wallets[0].address.slice(-8)}
                  </div>
                  <div className="text-sm  text-foreground font-bold flex items-center gap-2 mt-1">
              ...
              <Button
                className="h-16 px-12 text-sm font-bold tracking-widest uppercase bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl w-full max-w-sm shadow-[0_0_25px_-5px_hsl(var(--primary)/0.6)] transition-all"
              ...
              <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2 bg-background/50 px-4 py-2 rounded-full border border-border">
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <Button
          variant="outline"
          size="sm"
          className="text-[10px] font-bold tracking-widest uppercase border-border h-10 px-6"
      ...
      {!hasWallets ? (
        <div className="p-16 text-center border border-dashed border-border rounded-xl bg-card/20 flex flex-col items-center gap-5">
          ...
          <div>
            <p className="font-bold text-lg text-foreground tracking-widest uppercase">Database Empty</p>
