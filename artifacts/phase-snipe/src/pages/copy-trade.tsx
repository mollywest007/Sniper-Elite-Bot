...
import { Copy, Play, Pause, Trash2, Loader2, Link2, Radar } from "lucide-react";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-mono uppercase tracking-widest text-muted-foreground pl-1">Network Links</h2>
            <Button variant="outline" size="sm" className="h-8 font-mono text-xs tracking-widest uppercase border-border hover:bg-accent" onClick={() => queryClient.invalidateQueries({ queryKey: getListCopyTradesQueryKey() })}>Refresh Net</Button>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="p-16 text-center text-sm font-mono text-muted-foreground border border-dashed border-border rounded-xl bg-card/20 flex flex-col items-center gap-4">
              ...
              <p className="font-mono text-sm font-bold text-muted-foreground uppercase tracking-widest">No Active Links</p>
              <p className="font-mono text-xs text-muted-foreground/60">Initialize a new mirror connection via the panel →</p>
            </div>
          ) : (
                  ...
                  <div className="p-5 flex flex-col sm:flex-row justify-between gap-5">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        <span className="font-bold font-mono text-lg tracking-tight text-foreground">{ct.targetAlias || "Unnamed Node"}</span>
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-xs font-mono">
                        <div className="bg-background/50 rounded-lg p-2 border border-border/50">
                          <div className="text-muted-foreground uppercase text-[9px] tracking-widest mb-0.5">Strategy</div>
                        ...
                        <div className="bg-background/50 rounded-lg p-2 border border-border/50">
                          <div className="text-muted-foreground uppercase text-[9px] tracking-widest mb-0.5">Mirrored</div>
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="font-mono text-xs uppercase tracking-widest text-muted-foreground">Target Hash</FormLabel>
                        <FormControl>
                          <Input placeholder="Wallet address..." className="font-mono bg-background/50 h-12 border-border focus:border-primary/50 text-[10px] sm:text-xs" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="font-mono text-xs uppercase tracking-widest text-muted-foreground">Node Alias (Opt)</FormLabel>
                        <FormControl>
                          <Input placeholder="Alpha Whale" className="font-mono bg-background/50 h-12 border-border focus:border-primary/50" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <div className="bg-background/30 p-4 rounded-xl border border-border space-y-4">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-mono text-xs uppercase tracking-widest text-muted-foreground">Strategy</FormLabel>
                          <FormControl>
                            <select 
