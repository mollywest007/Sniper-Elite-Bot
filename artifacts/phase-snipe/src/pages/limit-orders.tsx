...
import { Target, Trash2, Loader2, Link2, Activity, Zap } from "lucide-react";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground pl-1">Armed Triggers</h2>
            <Button variant="outline" size="sm" className="h-8 text-[10px] font-bold tracking-widest uppercase border-border hover:bg-accent" onClick={() => queryClient.invalidateQueries({ queryKey: getListLimitOrdersQueryKey() })}>Sync Data</Button>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="p-16 text-center text-sm text-muted-foreground border border-dashed border-border rounded-xl bg-card/20 flex flex-col items-center gap-4">
              ...
              <p className="font-bold text-sm text-muted-foreground uppercase tracking-widest">No Active Conditions</p>
              <p className=" text-xs text-muted-foreground/60">Configure execution parameters via the panel →</p>
            </div>
          ) : (
                  ...
                  <div className="p-5 flex flex-col sm:flex-row justify-between gap-5">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        <span className="font-bold text-lg tracking-tight text-foreground">{order.tokenSymbol || "Unknown Asset"}</span>
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-xs ">
                        {order.takeProfitPercent && (
                          <div className="bg-background/50 rounded-lg p-2 border border-success/30">
                            <div className="text-muted-foreground font-bold uppercase text-[9px] tracking-widest mb-0.5">Take Profit</div>
                        ...
                        {order.stopLossPercent && (
                          <div className="bg-background/50 rounded-lg p-2 border border-destructive/30">
                            <div className="text-muted-foreground font-bold uppercase text-[9px] tracking-widest mb-0.5">Stop Loss</div>
                        ...
                        {order.trailingStopPercent && (
                          <div className="bg-background/50 rounded-lg p-2 border border-primary/30">
                            <div className="text-muted-foreground font-bold uppercase text-[9px] tracking-widest mb-0.5">Trailing</div>
                        ...
                        <div className="bg-background/50 rounded-lg p-2 border border-border/50">
                          <div className="text-muted-foreground font-bold uppercase text-[9px] tracking-widest mb-0.5">Auto Exec</div>
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Token address..." className=" font-bold bg-background/50 h-12 border-border focus:border-primary/50 text-[10px] sm:text-xs" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-bold text-[10px] uppercase tracking-widest text-muted-foreground">TP (%)</FormLabel>
                          <FormControl>
                            <Input type="number" step="0.1" className=" bg-background/50 h-12 text-primary font-bold border-border focus:border-primary/50" {...field} value={field.value ?? ""} placeholder="Opt" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-bold text-[10px] uppercase tracking-widest text-muted-foreground">SL (%)</FormLabel>
                          <FormControl>
                            <Input type="number" step="0.1" className=" bg-background/50 h-12 text-destructive font-bold border-border focus:border-destructive/50" {...field} value={field.value ?? ""} placeholder="Opt" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="font-bold text-[10px] uppercase tracking-widest text-muted-foreground">Trailing Stop (%)</FormLabel>
                        <FormControl>
                          <Input type="number" step="0.1" placeholder="Optional" className=" bg-background/50 h-12 text-primary font-bold border-border focus:border-primary/50" {...field} value={field.value ?? ""} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-xl border border-border p-4 bg-background/30">
                        ...
                        <div className="space-y-1.5 leading-none">
                          <FormLabel className="font-bold text-[10px] uppercase tracking-widest cursor-pointer text-foreground">
                            Auto Execution
                          </FormLabel>
