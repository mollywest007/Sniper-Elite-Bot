...
def kb_back(target: str, label: str = "◀️ Back") -> InlineKeyboardMarkup:
...
def kb_sniper(cfg: dict) -> InlineKeyboardMarkup:
    return kb(
        ...
        [
            btn("⏹ Stop Sniping" if cfg["sniping"] else "▶️ Start Sniping",
...
def kb_wallet() -> InlineKeyboardMarkup:
    return kb(
        [btn("💳 Deposit", "deposit:show"),  btn("↗️ Withdraw", "withdraw:start")],
...
def kb_sniper_edit(cfg: dict) -> InlineKeyboardMarkup:
    ...
    return kb(
        ...
        [btn("◀️ Sniper Panel", "sniper:panel")],
    )
...
def kb_alerts(user_id: int) -> InlineKeyboardMarkup:
    ...
    return kb(
        ...
        [btn("💳 Deposit", "alerts:type:deposit"), btn("↗️ Withdraw", "alerts:type:withdraw")],
